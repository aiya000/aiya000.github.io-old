...
tags: ラブライブ！で学ぶ, ことり、穂乃果と一緒に学ぶHaskell
---
# 前置き（海未ちゃん）
...
ことり「うっ、穂乃果ちゃんするどいね……実はHaskell、そのままじゃ日本語を表示できなくって。
　ある型を使えば問題ないんだけど、今回の内容とは外れちゃうから、これも今は無視してくれる……かな」  
穂乃果「ある型？　はーい」  
...
+ derivingできる型クラス
    - Show
        ...
        - toStringっぽい
    - Eq
        - そのデータ型とそのデータ型の等価不等価を取得することができる
        - `x == y` のようなことができる（equal）
        - `x /= y` のようなことができる（not equal）
    - Ord
        - そのデータ型とそのデータ型を比較できる
        - `x < y` のようなことができる
        - `x >= y` のようなことができる
...
ことり「それはね穂乃果ちゃん、いわゆる型クラスって言って、……ReadやEnumと合わせて解説していくね！」  
